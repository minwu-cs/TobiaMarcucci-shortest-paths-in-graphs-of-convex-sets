...
class HybridModelPredictiveController(object):
    ...
    def __init__(self, S, N, Q, R, P, X_N, method='ch', norm='two', noTerminal=False):
        ...
        if method == 'mld':
            self.prog = bild_mip_mld(S, N, Q, R, P, X_N, norm, noTerminal=noTerminal)

        # big-m method (Vielma 2015)
        elif method == 'bm':
            self.prog = bild_mip_bm(S, N, Q, R, P, X_N, norm, noTerminal=noTerminal)

        # convex hull method
        elif method == 'ch':
            self.prog = bild_mip_ch(S, N, Q, R, P, X_N, norm, noTerminal=noTerminal)

        # perspective formulation
        elif method == 'pf':
            self.prog = bild_mip_pf(S, N, Q, R, P, X_N, norm, noTerminal=noTerminal)
    ...
    def set_initial_condition(self, x0):
        for k in range(self.S.nx):
            self.prog.getVarByName('x@0[%d]'%k).LB = x0[k]
            self.prog.getVarByName('x@0[%d]'%k).UB = x0[k]
        self.prog.update()
    ...
    def feedforward(self, x0, gurobi_options={}, modelName="", noTerminal=False):
        ...
        if modelName != "" and self.prog.status not in [grb.GRB.OPTIMAL, grb.GRB.INFEASIBLE, grb.GRB.INF_OR_UNBD]:
            self.prog.write(modelName)
        
        return None #self.organize_result()
