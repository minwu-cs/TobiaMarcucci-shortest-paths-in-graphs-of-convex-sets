...
def add_linear_inequality(prog, x, y, name=""):
    assert x.size == y.size
    return [prog.addConstr(x[k] - y[k] <= 0., name) for k in range(x.size)] # sometimes x is a vector of floats: gurobi raises errors if variables are not in the lhs

def add_linear_equality(prog, x, y, name=""):
    assert x.size == y.size
    return [prog.addConstr(x[k] - y[k] == 0., name) for k in range(x.size)] # sometimes x is a vector of floats: gurobi raises errors if variables are not in the lhs
