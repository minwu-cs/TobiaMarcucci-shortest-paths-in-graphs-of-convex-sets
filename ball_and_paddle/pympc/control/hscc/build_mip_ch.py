...
def bild_mip_ch(S, N, Q, R, P, X_N, norm, noTerminal):
    ...
    for t in range(N):
        ...
        # initial conditions
        if t == 0:
            x = add_vars(prog, nx, name='x@0')

        # stage variables
        else:
            x = x_next
        x_next = add_vars(prog, nx, name='x@%d'%(t+1))
        u = add_vars(prog, nu, name='u@%d'%t, lb=-30, ub=30)
        d = add_vars(prog, nm, lb=[0.]*nm, name='d@%d'%t)

        # auxiliary continuous variables for the convex-hull method
        y = [add_vars(prog, nx, name='y{}@{}'.format(i, t)) for i in range(nm)]
        v = [add_vars(prog, nu, name='v{}@{}'.format(i, t)) for i in range(nm)]
        z = [add_vars(prog, nx, name='z{}@{}'.format(i, t)) for i in range(nm)]
